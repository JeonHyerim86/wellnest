...
### 4.19 Skeleton (.pen `C/Skeleton`)

데이터를 기다리는 동안 **실제 레이아웃과 같은 골격**을 먼저 그린다. 스피너를 쓰지 않는 이유는
스피너가 "무언가 로딩 중"이라는 사실만 알려줄 뿐 화면이 어떤 모양이 될지 예고하지 못하고,
데이터가 도착하는 순간 레이아웃이 튀기 때문이다.

| 토큰 | 값 | 용도 |
| --- | --- | --- |
| `skeleton` | `#E2DFD4` | 기본 골격. `cream-50` 배경과 흰 카드 위 모두에서 읽힌다 |
| `skeleton-strong` | `#D6D2C4` | 골격 블록 **위에** 얹는 요소 (달성률 카드 내부 등) |

- 애니메이션: 불투명도 `1 → 0.45`, `1.6s`, `ease-in-out`, 무한 반복 (`--animate-skeleton`)
- `prefers-reduced-motion: reduce` 에서는 펄스를 멈춘다
- 변형 (모두 `C/Skeleton` 인스턴스의 크기·radius 오버라이드)
...
| Circle | 44 × 44 | full |
| Tile | 100 × 62 | 16 |
| Block | 150 × 76 | `radius-card` |

**사용 규칙**

- 골격의 여백·크기는 대체할 실제 컴포넌트와 **동일하게** 맞춘다 (레이아웃 시프트 0)
- 목록 골격은 항목마다 제목 폭을 다르게 준다. 모두 같으면 기계적으로 보인다
- 골격 전체를 `role="status" aria-busy="true"` 로 감싸고, 개별 블록은 `aria-hidden` 처리한다
- 로딩 화면: `Screen · Today / Routines / Logs / Profile (Loading)`
